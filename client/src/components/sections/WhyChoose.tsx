...
function useStatsCounters(stats: Stat[], inView: boolean) {
  const count1 = useCounter(stats[0].number, inView);
  const count2 = useCounter(stats[1].number, inView);
  const count3 = useCounter(stats[2].number, inView);
...
const WhyChoose = () => {
  ...
  const counts = useStatsCounters(stats, inView);
